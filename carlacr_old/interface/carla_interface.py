...
from carlacr.config.set_configs import set_configs
from carlacr.config.configuration import Configuration
...
class CarlaInterface:
    ...
    def __init__(self, open_drive_map_path: str, cr_scenario_file_path: str = None, config: Configuration = None):
        """
        Constructor of CarlaInterface.

        :param cr_scenario_file_path: full path & filename to a CommonRoad XML-file
        :param open_drive_map_path: full path & filename to OpenDRIVE map for the simulation or name of CARLA default map
        """
        self.map = open_drive_map_path
        if config is None:
            self._config = set_configs()
        else:
            self._config = config
        self.client = carla.Client(config.carla_config.host, config.carla_config.port)
        self.cr_path = cr_scenario_file_path
