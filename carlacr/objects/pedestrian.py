...
class PedestrianInterface(ActorInterface):
    ...
    def _spawn(self, time_step: int):
        ...
        if self._config.controller_type is PedestrianControlType.AI:
            ...
            ai_actor = self._world.spawn_actor(walker_controller_bp, carla.Transform(), actor)
            ai_actor.start()
            position = self._cr_obstacle.prediction.trajectory.final_state.position
            location = carla.Location(x=position[0], y=-position[1], z=0.5)
            ai_actor.go_to_location(location)
            ai_actor.set_max_speed(max([state.velocity for state in self._cr_obstacle.prediction.trajectory.state_list]))
        self._actor = actor
